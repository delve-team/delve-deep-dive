...
class LatentRepresentationCollector:
    ...
    def _record_stat(self, activations_batch: torch.Tensor, layer: Module, training_state: str):
        ...
        if not self.save_instantly:
            ...
            #df.to_csv(savepath, mode='a' if not exists(savepath) else 'w+', header=not exists(savepath))
            #if exists(savepath+'.npy'):
            #    data = np.load(savepath+'.npy')
            #    new_data = np.vstack((data, batch))
            #else:
            #    print('Did not find prexisting file, moving on...')
            #    new_data = batch
